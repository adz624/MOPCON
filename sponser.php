<?php
  $page = "sponser";
?>
<html lang="zh-TW" class="">
  <head><?php include_once("header.php");?></head>
  <body data-twttr-rendered="true" data-spy="scroll" data-target=".subnav" data-offset="50">
    <div class="container" id="all">
      <?php include_once("navbar.php");?>
      <div class="article">
	      <div id="sponsorList">
		<div class="main">
			<h3>主辦單位：&nbsp;&nbsp;&nbsp;經濟部加工出口區管理處</h3>
			<p>感謝經濟部加工出口區對產業與社群發展的支持</p>
		</div>
		<div class="second">
			<h3>執行單位：</h3>
			<ul class="thumbnails">
				<li class="thumbnail"><a href="https://www.facebook.com/stipc.nsysu"><img alt="NSYSU STIPC logo" src="img/logo/nsysu_stipc-logo.png"></a></li>
				<li class="thumbnail"><a href="http://www.facebook.com/groups/KSDGroup/"><span class="fontLogo">KSDG</span></a></li>
				<li class="thumbnail"><a href="http://www.facebook.com/groups/cocoaheads.kaohsiung/"><img alt="CocoaHeads Kaohsiung logo" src="img/cocoaheads.jpg"></a></li>				
				<li class="thumbnail"><a href="http://www.facebook.com/groups/mosut"><img alt="MOSUT logo" src="img/mosut.png" /></a></li>
				
			</ul>
		</div>
		<div class="second">
			<h3>Hacker級贊助商</h3>
			<p>感謝以下各贊助單位</p>
			<ul class="thumbnails">
                            <li class="thumbnail"><a rel="external" href="http://www.acomotech.com/"><img src="img/logo/acomotech-logo.png" alt="杏昕科技" /></a>
                            <li class="thumbnail"><a rel="external" href="http://www.EZoUI.com/"><img src="img/logo/Ezo-logo.png" alt="中冠資訊" /></a>
                        </ul>
                </div>
		<div class="second">
			<h3>Geek級贊助商</h3>
			<p>感謝以下各贊助單位</p>
			<ul class="thumbnails">
                            <li class="thumbnail"><a rel="external" href="http://www.kkbox.com"><img src="img/logo/kkbox-logo.png" alt="KKBOX" /></a>
                            <li class="thumbnail"><a rel="external" href="http://www.mobilogics.com.tw"><img src="img/logo/mobilogics-logo.png" alt="明錄國際" /></a>
                            <li class="thumbnail"><a rel="external" href="http://www.moko365.com"><img src="img/logo/moko-logo.png" alt="Moko365" /></a>
                            <li class="thumbnail"><a rel="external" href="http://www.moregeek.com"><img src="img/logo/moregeek-logo.png" alt="摩鉅科技" /></a>
                            <li class="thumbnail"><a rel="external" href="http://www.wiwistock.com.tw"><img src="img/logo/wiwistock-logo.png" alt="鈞達資訊" /></a>
			</ul>
		</div>
<!--		<div class="special">
			<h3>特別感謝</h3>
			<p>感謝以下各單位之協助</p>
		</div>
-->
	</div>
</div>
    </div>
    <?php include_once("footer.html");?>
  </body>
</html>
